document.addEventListener('DOMContentLoaded', () => {
    ...
    if (adminLoginForm) {
        adminLoginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('https://cmr-canteen-vkth.onrender.com/:4000/admin', {
                    method: 'POST',
                    headers: {
    ...
    if (adminSignupForm) {
        adminSignupForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch('https://cmr-canteen-vkth.onrender.com/:4000/admin/add', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const addDishForm = document.getElementById('dish-form');
        ...
        if (addDishForm) {
            addDishForm.addEventListener('submit', async (e) => {
                ...
                try {
                    const response = await fetch('https://cmr-canteen-vkth.onrender.com/:4000', {
                        method: 'POST',
                        headers: {
                            'Authorization': `Bearer ${token}` // Add the token to the Authorization header
                        },
